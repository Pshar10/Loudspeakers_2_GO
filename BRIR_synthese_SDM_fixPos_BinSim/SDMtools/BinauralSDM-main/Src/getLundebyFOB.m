...
% bands.
    ...
    [G,g,f1,f2] = oneOver_n_OctBandFilter(2*snfft, bandsPerOctave, Fs, minFreq , maxFreq);
    ...
    for iOb = 1:size(g,2)
        ...
        %Select RT estimation
        %RTsOLD(iOb) = getLundebyRT30(sigFilt, Fs,0.02,Tn)
        %RTs(iOb) = getLundebyRT30(sigFilt, Fs,0.02,Tn);
        RTs(iOb) = getRTw(sigFilt, Fs,0.02,Tn,fcentre(iOb));
