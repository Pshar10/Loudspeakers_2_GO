function hpol = dirplot(theta,rho,line_style,params,yTitle)
...
%
%   ----- ITA-Toolbox: Modified/Bugfix rho datascaling, January 2014 -----
%   ----- see comment "begin ITA-Toolbox ..." below                  -----
%
...
if exist('params','var')
    if length(params) ~= 3
        error('Argument PARAMS must be a 3-element vector: [RHOMAX RHOMIN RHOTICKS YTITLE].')
    end
...
if hold_state && exist('params','var')
...
if ~hold_state
    ...
    for i=click:click:rngdisp
        ...
        if i < rngdisp
            ...
            text(i*c88,i*s88, ...
                [ticktext,yTitle],'verticalalignment','bottom',...
                'handlevisibility','off','fontsize',8)
        end
        if fullplot
            if i < rngdisp
...
%begin ITA-Toolbox - extension (tumbraegel): dirplot has error by plotting
%values outside limits rmax, rmin without this
%bugfix: do not plot data outside:
MinRho= find(rho<rmin);
MaxRho = find(rho>rmax);
rho([MinRho; MaxRho])  = NaN*ones(size([MinRho; MaxRho]));
theta([MinRho; MaxRho]) = NaN*ones(size([MinRho; MaxRho]));
%end ITA-Toolbox - extension
