...
if ~ita_preferences('colorDisp') || ~isempty(javachk('desktop'))
    disp(message,varargin{:})
    return;
end

% The following is for debug use only:
%global docElement txt el
if ~exist('elementHandle','var') || isempty(elementHandle),  elementHandle=handle([]);  end  %#ok mlint short-circuit error ("used before defined")
...
if bolFlag && ~underlineFlag
    ...
    %disp(' '); dumpElement(docElement)
    elementHandle(end+1) = handle(docElement);
end
...
% The following is for debug use only:
elementHandle(end+1) = handle(docElement);  %#ok used in debug only
