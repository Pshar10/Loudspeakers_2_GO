...
if isa(varargin{1},'itaSuper')
    allIsWell = all(isTime(varargin{1}));
    if isa(varargin{2},'itaSuper')
        allIsWell = allIsWell && all(isTime(varargin{2}));
    end
end

if allIsWell
    varargout{1} = times@itaSuper(varargin{:});
else
    error('itaResult.times:your signals are not in the time domain, use operator * instead!');
end
