classdef  itaWaitbar < itaHandle
    ...
    %itaWaitbar - class for progress bar
    %
    %
    ...
    methods
        ...
        function showTotalTime(this) 
            ...
            if this.iLoop
                timeStr = ['total time: ' datestr(   now - this.startTimeWaitbar ,'HH:MM:SS')];
            else
                timeStr = '';
            end
        ...
        function updateBar(this, updateMessage)
            ...
            for iNesting = 1:this.nNestings
                ...
                set(this.txtHandle(iNesting), 'string', sprintf('%s  %i of %i ', this.loopNameCell{iNesting}, this.iLoopForEveryNesting{iNesting}, this.nLoops(iNesting)));
            end
            
            % update time left
            
            if isnan(this.startTimeLastLoop)
                this.startTimeLastLoop  = now;
                remainingTimeStr        = '';
%             elseif this.iLoopIntern > 1 && ~isnan(this.startTimeLastLoop)
            else
                startTimeThisLoop       = now;
                timeForLastLoop         = startTimeThisLoop - this.startTimeLastLoop;
                this.startTimeLastLoop  = startTimeThisLoop;
                timeLeft = timeForLastLoop*(prod(this.nLoopsIntern)-this.iLoopIntern+1);
        ...
        function pauseCalculation(this, self, eventData)
            if get(self, 'value') % pause on
                this.startTimeLastLoop = nan; % no estimation possible for next loop
